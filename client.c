...
void execution( int internet_socket, struct sockaddr * internet_address, socklen_t internet_address_length )
{
	...
	char aantalPackets[1000];
	int32_t packets=0;
	packets = atoi(aantal_packets);
	printf("Geef een aantal pakketen in: ");
	gets(aantalPackets);
	
	number_of_bytes_send = sendto( internet_socket, aantalPackets, strlen(aantalPackets), 0, internet_address, internet_address_length );
	if( number_of_bytes_send == -1 )
	{
		perror( "sendto" );
	}
	
	packets = atoi(aantalPackets);
